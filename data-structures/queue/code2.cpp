/*
 * Queue Implementation in C++ (Front Pointer , Rear Pointer Varying)
 * ------------------------------------------------------------------------
...
public:
    ...
    int dequeue()
    {
        if (FRONT > REAR)
        {
            cout << "Underflow: Queue is empty. No element to pop." << endl;
            display();
            return -1;
        }
        else
        {
            ...
            arr[FRONT] = 0;   // for the indication of deletion
            FRONT++;
            display();
            return poppedValue;
        }
